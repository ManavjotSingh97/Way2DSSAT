...
## ```Way2DSSAT.SOL()``` : Creates DSSAT Soil File (SG.SOL)
 Use `Way2DSSAT.SOL("path/to/you/gssurgo.csv")` to create a soil file first. The funciton will save the soil file as "SG.SOL".  
 If you have multiple soil types in the csv exported from GSSURGO data, different soil types will be created in the same SG.SOL file with the soil ID created using the MUKEY "SGG"+"MUKEY".eg. GSS2733103.
 
Since the function is designed to handle CSV files generated by the gSSURGO soil database, ***it is aware of the column names generated by the gSSURGO ArcMap toolbox***. Thus, no need to remove any columns whose column name contains the 'pct' string. 
...
**Columns required**:
 ...
 It also returns a dictionary of soil depth(SDUL), which will be used as initial conditions in the Xfile. A string of MUKEYS of all the soil profiles created. **Important** Save the returned dictionary to a variable
For eg.
